# [3484. 设计电子表格][link] (Medium)

[link]: https://leetcode.cn/problems/design-spreadsheet/

电子表格是一个网格，它有 26 列（从 `'A'` 到 `'Z'`）和指定数量的 `rows`。每个单元格可以存储一个 0 到 
10 之间的整数值。

请你实现一个 `Spreadsheet` 类：

- `Spreadsheet(int rows)` 初始化一个具有 26 列（从 `'A'` 到 `'Z'`）和指定行数的电子表格。所有单元格
最初的值都为 0 。
- `void setCell(String cell, int value)` 设置指定单元格的值。单元格引用以 `"AX"` 的格式提供（例如， 
`"A1"`， `"B10"`），其中字母表示列（从 `'A'` 到 `'Z'`），数字表示从 **1** 开始的行号。
- `void resetCell(String cell)` 重置指定单元格的值为 0 。
- `int getValue(String formula)` 计算一个公式的值，格式为 `"=X+Y"`，其中 `X` 和 `Y` **要么** 是单元
格引用，要么非负整数，返回计算的和。
...
- `1 <= rows <= 10³`
- `0 <= value <= 10⁵`
- 公式保证采用 `"=X+Y"` 格式，其中 `X` 和 `Y` 要么是有效的单元格引用，要么是小于等于 `10⁵` 的 **非负
** 整数。
