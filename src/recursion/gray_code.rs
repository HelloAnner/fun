use super::Solution;
/**
n 位格雷码序列 是一个由 2n 个整数组成的序列，其中：
每个整数都在范围 [0, 2n - 1] 内（含 0 和 2n - 1）
第一个整数是 0
一个整数在序列中出现 不超过一次
每对 相邻 整数的二进制表示 恰好一位不同 ，且
第一个 和 最后一个 整数的二进制表示 恰好一位不同
给你一个整数 n ，返回任一有效的 n 位格雷码序列 。

输入：n = 2
输出：[0,1,3,2]
解释：
[0,1,3,2] 的二进制表示是 [00,01,11,10] 。
- 00 和 01 有一位不同
- 01 和 11 有一位不同
- 11 和 10 有一位不同
- 10 和 00 有一位不同
[0,2,3,1] 也是一个有效的格雷码序列，其二进制表示是 [00,10,11,01] 。
- 00 和 10 有一位不同
- 10 和 11 有一位不同
- 11 和 01 有一位不同
- 01 和 00 有一位不同

https://leetcode.cn/problems/gray-code/description/


 */
impl Solution {
    #[allow(dead_code)]
    pub fn gray_code(n: i32) -> Vec<i32> {
        (0..1 << n).map(|i| (i >> 1) ^ i).collect()
    }
}

#[allow(dead_code)]
fn generate_gray_code(n: usize) -> Vec<String> {
    match n {
        0 => vec!["0".to_string()],
        1 => vec!["0".to_string(), "1".to_string()],
        _ => {
            let prev_gray_code = generate_gray_code(n - 1);
            let mut gray_code = Vec::with_capacity(prev_gray_code.len() * 2);

            // Append '0' to the beginning of each code in the previous gray code
            for code in &prev_gray_code {
                gray_code.push(format!("0{}", code));
            }

            // Append '1' to the beginning of each code in the reversed previous gray code
            for code in prev_gray_code.iter().rev() {
                gray_code.push(format!("1{}", code));
            }

            gray_code
        }
    }
}
